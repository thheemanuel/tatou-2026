...
def create_app():
    ...
    app.config["RMAP_SERVER_PUB"] = Path(os.environ["RMAP_SERVER_PUB"])
    
    app.config["RMAP_SERVER_PRIV"] = Path(os.environ["RMAP_SERVER_PRIV"])
    ...
    rmap_server.loadIdentities(
        app.config["RMAP_CLIENT_KEYS"]
    )
    ...
    @require_auth #in order to delete documents, you should need to authorize yourself.
    ...
    # this function will need to be called after another group has successfully authenticated using rmap
    
    # we will need the identity of the group that requested the document.
    
    #we will also need the link generated by rmap for this request
    
    #we need this function to do four things:
    
    # find our confidential pdf in the database,
